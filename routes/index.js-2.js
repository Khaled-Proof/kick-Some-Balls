...
//khaled start
    ...
    const Promise = playersdata.find({ }, async function (err, results) {
            ...
            for (const element1 of results) {
                ...
                if(name2 !=name3) {
                   ...
                   // n3names.push(name3);
                    ...
                    p23lw.push(lwtotal);
                    //Team Striche
                    const teamstrich1 = await userdata.find({$and: [{$and: [{Player2: name2}, {Player1: name3}]}, { $expr: { $eq: ['$team2', '0'] }}]});
                    const teamstrich2 = await userdata.find({$and: [{$and: [{Player1: name2}, {Player2: name3}]}, { $expr: { $eq: ['$team2', '0'] }}]});
                    const teamstrich3 = await userdata.find({$and: [{$and: [{Player3: name2}, {Player4: name3}]}, { $expr: { $eq: ['$team1', '0'] }}]});
                    const teamstrich4 = await userdata.find({$and: [{$and: [{Player4: name2}, {Player3: name3}]}, { $expr: { $eq: ['$team1', '0'] }}]});
                   ...
                   // const le =Object.values(totalstriche);
                    ...
                    //teamstrich.push(teamstrichegesamet);
        ...
        playersdata.find().lean()
            .then(function (doc){
                res.render('charts', {item:doc,title: 'gizn&Khaled Kicker Project', condition: true, namearray: finalResults,wonarray:wonResults,lostarray:lostResults,wper:persentagew,lper:persentagel,total:totalplayed,pn2:n2names,gwin:p23wons,glos:p23los,glw:p23lw,st:strich,stt:teamstrich
            });
